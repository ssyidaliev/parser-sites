...
class CommercialService:
    ...
    def check_record(cls, **filters):
        return conn.query(Commercial).filter_by(**filters).all()

    @classmethod
    def get(cls, **filters):
        return conn.query(Commercial).filter_by(**filters).first()

    @classmethod
    def create_record(cls, **filters):
        record = Commercial(**filters)
        conn.add(record)
        conn.commit()
        return True

    @classmethod
    def add_image(cls, car_id: int, image_list: list, key: str):
        try:
    ...
    def get_image(cls, key: str, car_id: int):
        image = conn.query(CommercialImage.image).filter_by(commercial_id=car_id).all()
        try:
            mkdir(f'images/{key}')
